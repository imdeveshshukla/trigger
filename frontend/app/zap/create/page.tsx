...
function Modal({ index, onSelect, availableItems }: { index: number, onSelect: (props: null | { name: string; id: string; metadata: any; }) => void, availableItems: {id: string, name: string, img: string;}[] }) {
        ...
        <div className="relative p-4 w-full max-w-2xl max-h-full">
            <div className="relative bg-white rounded-lg shadow ">
                ...
                <div className="p-4 md:p-5 space-y-4">
                    ...
                    {(step === 1 && selectedAction?.name === "Razorpay-link") && <RZPSelector setMetadata={(metadata) => {
                        onSelect({
                            ...selectedAction,
                            metadata
                        })
                    ...
                    {(step === 1 && selectedAction?.name === "Whatsapp") && <WhatsappSelector setMetadata={(metadata) => {
                        onSelect({
                            ...selectedAction,
                            metadata
                        })
                    }} />}
                    {step === 0 && <div>{availableItems.map(({id, name, img}) => {
                            return <div onClick={() => {
                                if (isTrigger) {
                                    onSelect({
                                        id,
                                ...
                                <img src={img} width={30} className="rounded-full" /> <div className="flex flex-col justify-center"> {name} </div>
                            </div>
                        })}</div>}                    
                </div>
            </div>
...
function WhatsappSelector({setMetadata}: {
    setMetadata: (params: any) => void;
}) {
    const [amount, setAmount] = useState("");
    const [address, setAddress] = useState("");    
    ...
    return <div>
        <Input label={"Phone"} type={"text"} placeholder="Phone" onChange={(e) => setAddress(e.target.value)}></Input>
        <Input label={"Message"} type={"text"} placeholder="Message" onChange={(e) => setAmount(e.target.value)}></Input>
        <div className="pt-4">
        <PrimaryButton onClick={() => {
            setMetadata({
                phone:address,
...
function RZPSelector({setMetadata}: {
    setMetadata: (params: any) => void;
}) {
    const [amount, setAmount] = useState("");
    const [address, setAddress] = useState("");    
    ...
    return <div>
        <Input label={"Name"} type={"text"} placeholder="Name" onChange={(e) => setName(e.target.value)}></Input>
        <Input label={"Phone Number"} type={"text"} placeholder="Phone" onChange={(e) => setAddress(e.target.value)}></Input>
        <Input label={"Email Id"} type={"text"} placeholder="Email" onChange={(e) => setEmail(e.target.value)}></Input>
        <Input label={"Amount"} type={"text"} placeholder="To" onChange={(e) => setAmount(e.target.value)}></Input>
        <div className="pt-4">
        <PrimaryButton onClick={() => {
            setMetadata({
